...
import { generateReport } from "../report-engine/index.js";
import { getApiKey, setApiKey, clearApiKey } from "../configs/store.js";
import type { ReportInput } from "../report-engine/types.js";
...
program
  .command("report")
  .description("Generate a structured report for any topic (disease, book, competitive analysis, etc.)")
  .requiredOption("--report-type <type>", "Report type matching a folder in /configs (e.g. disease_report, book_report)")
  .requiredOption("--name <name>", "Primary subject of the report")
  .option("--description <description>", "Brief context or background about the subject")
  .option("--depth <depth>", "Content depth: short | medium | detailed | academic", "detailed")
  .option("--style <style>", "Tone: clinical | executive | academic | literary | simplified", "clinical")
  .option("--author <author>", "Author name (for book reports)")
  .option(
    "--formats <formats>",
    "Comma-separated output formats: md,docx,pdf,pptx",
    "md,docx,pdf,pptx"
  )
  .action(async (options) => {
    const {
